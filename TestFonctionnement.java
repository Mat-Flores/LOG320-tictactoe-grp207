...
public class TestFonctionnement {
    ...
    public static void main(String[] args) {
        TestFonctionnement test = new TestFonctionnement();
        test.testRandom(500);
        test.testVersus(500);
    ...
    public void testRandom(int attempts) {
        Board board = new Board();
        int lossCount = 0, gameResult;
        ...
        for (int i = 0; i < attempts; i++) {
            while (!isGameOver(board)) {
                if (cpuTurn && i % 2 == 0) {
                    nextMove = cpu.getNextMoveMinMax(board).getFirst();
                    board.play(nextMove, Mark.O);
                } else if (cpuTurn) {
                    nextMove = cpu.getNextMoveAB(board).getFirst();
                    board.play(nextMove, Mark.O);
                } else {
                    ArrayList<Move> possibleMoves = board.generatePossibleMoves();
                    nextMove = possibleMoves.get((int) (Math.random() * possibleMoves.size() - 1));
                    board.play(nextMove, Mark.X);
                }
                cpuTurn = !cpuTurn;
            }
            gameResult = board.evaluate(Mark.O);
            if (gameResult == -100) {
                lossCount++;
            }
            board = new Board();
        ...
        if (lossCount == 0) {
            System.out.println("TEST RANDOM REUSSI");
        } else {
            System.out.println("TEST RANDOM ECHEC");
        }
    }

    // test qui verifie si nos deux algorithmes fonctionnent, en les mettant l'un contre l'autre.
    // devrait TOUJOURS donner un match nul
    public void testVersus(int attempts) {
        Board board = new Board();
        int tieCount = 0, gameResult;
        CPUPlayer cpuMM = new CPUPlayer(Mark.O);
        CPUPlayer cpuAB = new CPUPlayer(Mark.X);
        boolean alphaBeta = false; // est-ce le tour de cpuAB
        Move nextMove;
        for (int i = 0; i < attempts; i++) {
            while (!isGameOver(board)) {
                if (alphaBeta) {
                    nextMove = cpuMM.getNextMoveMinMax(board).getFirst();
                    board.play(nextMove, Mark.O);
                } else {
                    nextMove = cpuAB.getNextMoveAB(board).getFirst();
                    board.play(nextMove, Mark.X);
                }
                alphaBeta = !alphaBeta;
            }
        ...
        if (tieCount == attempts) {
            System.out.println("TEST VERSUS REUSSI");
        } else {
            System.out.println("TEST VERSUS ECHEC");
        }
    }
}